...
Last updated by Task 035 weighted yoga strength + cancellations (2026-05-11, branch `development`).
...
- **Tests:** 50 test files, 548 tests + 87 subtests, all passing. Coverage **94.11%** (`--cov=mayaastrolib --cov-fail-under=80`); `accidental.py` 100%, `temperament.py` 99%. **Status:** Phase 2 + 027–035 done. `mayaastrolib/vedic/` = 12 modules. Remaining: full six-fold Shadbala yoga strength (the accidental-dignity-weighted `yoga_strength_weighted` is in) · the rest of the ~50 Sahams + the 16 Tajika yogas · full KP horary chart with cusps · type hints on the public API · Ashtakavarga alternate shodhana variants — Tasks 036+.
