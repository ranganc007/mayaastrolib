...
Last updated by Task 032 lesser yogas + yoga strength (2026-05-11, branch `development`).
...
- **Tests:** 48 test files, 528 tests + 87 subtests, all passing. Coverage 92.10% (`--cov=mayaastrolib --cov-fail-under=80`). **Status:** Phase 2 + 027–032 done — the brief's entire phase-3 depth-fill list is shipped. `mayaastrolib/vedic/` = 12 modules. What remains is genuine long-tail polish (classical Shadbala-weighted yoga strength, the rest of the ~50 Sahams + the 16 Tajika yogas, a full KP horary chart with cusps, finer Neecha-Bhanga/Gaja-Kesari cancellation rules) — none blocking, all scoped in CHANGELOG/docstrings.
