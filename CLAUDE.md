...
- **Docs:** `docs/source/` builds an autodoc API reference from `__all__`. The gate is `python -m sphinx -W -b html docs/source docs/_build` — **zero warnings required**. Install with `pip install -e ".[docs]"`. Enabling the ReadTheDocs project is a one-time dashboard step (see `docs/RELEASING.md`).
