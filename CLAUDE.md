...
Last updated by Task 027 zodiac-aware predictives (2026-05-11, branch `development`).
...
- **Tests:** 44 test files, 461 tests + 87 subtests, all passing (Task 027 added 11 sidereal-predictives tests). Coverage 91.72% (`--cov=mayaastrolib --cov-fail-under=80`). **Status:** Phase 2 + 027 done. `mayaastrolib/vedic/` = 10 modules; `Chart.solarReturn`/`profected` now zodiac-aware (sidereal works correctly); `Chart.directions()` raises on sidereal. Remaining phase-3 depth-fill: full 50-Saham table, Harsha/Panchavargiya Bala, Tajika aspects, KP sub-sub-lord/horary/Ruling-Planets, Ashtakavarga shodhana/kakshya, lesser yogas, yoga strength — being worked through as Tasks 028–032.
