...
Last updated by Task 034 AccidentalDignity score-rule refactor (2026-05-11, branch `development`).
...
- **Tests:** 50 test files, 542 tests + 87 subtests, all passing. Coverage **94.07%** (`--cov=mayaastrolib --cov-fail-under=80`); `accidental.py` 100%, `temperament.py` 99%. **Status:** Phase 2 + 027–034 done. `mayaastrolib/vedic/` = 12 modules. Remaining: classical Shadbala-weighted yoga strength · the rest of the ~50 Sahams + the 16 Tajika yogas · full KP horary chart with cusps · finer Neecha-Bhanga/Gaja-Kesari cancellation · type hints on the public API · Ashtakavarga alternate shodhana variants — Tasks 035+.
