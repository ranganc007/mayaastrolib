...
Last updated by Task 036 Ashtakavarga shodhana variants (2026-05-11, branch `development`).
...
- **Tests:** 51 test files, 553 tests + 87 subtests, all passing. Coverage **94.12%** (`--cov=mayaastrolib --cov-fail-under=80`); `accidental.py` 100%, `temperament.py` 99%. **Status:** Phase 2 + 027–036 done. `mayaastrolib/vedic/` = 12 modules. Remaining: full six-fold Shadbala yoga strength · the rest of the ~50 Sahams + the 16 Tajika yogas · full KP horary chart with cusps · type hints on the public API — Tasks 037+.
