...
Last updated by Task 033 per-factor tests for smoke-only modules (2026-05-11, branch `development`).
...
- **Tests:** 50 test files, 541 tests + 87 subtests, all passing. Coverage **94%** (`--cov=mayaastrolib --cov-fail-under=80`); `accidental.py` and `temperament.py` are now 100%/99% (was 84%/80%). **Status:** Phase 2 + 027–033 done. `mayaastrolib/vedic/` = 12 modules. Remaining: classical Shadbala-weighted yoga strength · the rest of the ~50 Sahams + the 16 Tajika yogas · full KP horary chart with cusps · finer Neecha-Bhanga/Gaja-Kesari cancellation · `accidental.py` getScoreProperties rule-table refactor · type hints on the public API · Ashtakavarga alternate shodhana variants — being worked through as Tasks 034+.
