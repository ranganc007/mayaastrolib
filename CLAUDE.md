...
Last updated by Task 028 fuller Tajika Saham table (2026-05-11, branch `development`).
...
- **Tests:** 44 test files, 462 tests + 87 subtests, all passing. Coverage 91.70% (`--cov=mayaastrolib --cov-fail-under=80`). **Status:** Phase 2 + 027 + 028 done. `mayaastrolib/vedic/` = 10 modules; `Chart.solarReturn`/`profected` zodiac-aware; `tajika.sahams()` returns 14 Sahams (data-driven table). Remaining phase-3 depth-fill: rest of the ~50-Saham list, Harsha/Panchavargiya Bala, Tajika aspects, KP sub-sub-lord/horary/Ruling-Planets, Ashtakavarga shodhana/kakshya, lesser yogas, yoga strength — Tasks 029–032.
