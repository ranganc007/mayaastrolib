...
Last updated by Task 037 public-API type hints — geopos.py (2026-05-11, branch `development`).
...
- **Tests:** 51 test files, 553 tests + 87 subtests, all passing. Coverage **94.12%** (`--cov=mayaastrolib --cov-fail-under=80`); `accidental.py` 100%, `temperament.py` 99%. **Status:** Phase 2 + 027–037 done. `mayaastrolib/vedic/` = 12 modules. **Type-hint pass:** `geopos.py` done; `datetime.py`/`aspects.py`/`chart.py`/`object.py` still to do. **Other remaining (genuine open work, none blocking):** full six-fold Shadbala yoga strength (accidental-dignity-weighted version is in) · the rest of the ~50 Sahams + the other 13 Tajika yogas (source-variant — needs reference texts) · full KP horary chart with house cusps (the Lagna sub-lord chain is in). PyPI release and the cross-repo oracle-parity tests are out of scope per the user.
