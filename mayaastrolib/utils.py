...
def eqCoords(lon, lat):
    ...
    decl = math.asin(
        math.sin(_epson) * math.sin(_lambda) * math.cos(_beta) + math.cos(_epson) * math.sin(_beta)
    )
    ...
    if abs(angle.closestdistance(lon, 0)) < 5 or abs(angle.closestdistance(lon, 180)) < 5:
        ...
        b = math.cos(_epson) * math.sin(_lambda) * math.cos(_beta) - math.sin(_epson) * math.sin(
            _beta
        )
